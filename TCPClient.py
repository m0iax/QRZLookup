...
if os.path.isfile(configfilename):
    ...
    servertcpport = int(config.get('NETWORK', 'servertcpport'))

listen = (serverip, servertcpport)
...
class Server(threading.Thread):
    ...
    def process(self, message):
        ...
        if typ==TYPE_CALL_ACTIVITY:
            print('Got call activity list from JS8Call')
            self.heardList=params
            self.listrequested=False

        if not typ:
            return

        if not self.listrequested:
            self.messageType=TYPE_GET_CALL_ACTIVITY
            self.messageText=''
            self.listrequested=True
    ...
    def run(self):
        print('tcp listening on', ':'.join(map(str, listen)))
